...
#stack is used to check state of compiler, matching a terminal on top will pop top of the stack out
#stating with EOF (End of File)
stack_LL1 = ["EOF"]

#alphabet set stores english alphabets
alphabet = set(e for e in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz")

#terminal set
terminals = set(e for e in ["+", "-", "IF", "<", "=", "PRINT", "GOTO", "STOP", "EOF"])

#numbers came from the assignment
#set of these numbers will be our desired final output
bcode_type ={ 
    "#line" : 10,
    "#id" : 11,
    "#const" : 12,
...
#this shows first set and follow set
parsing_table = {
...
# symbol in parsing table -> non-terminals
# symbol not in non-terminals -> terminals
def is_terminal(symbol):
    return not symbol in parsing_table

# tokenizing is simply a split by white space
def tokenize(line):
